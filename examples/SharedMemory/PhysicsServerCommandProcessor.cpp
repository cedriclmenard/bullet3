...
bool PhysicsServerCommandProcessor::processRequestCastContactpointInformationCommand(const struct SharedMemoryCommand& clientCmd, struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient, int bufferSizeInBytes)
{
    bool hasStatus = true;
    BT_PROFILE("CMD_REQUEST_CAST_CONTACT_POINT_INFORMATION");
    SharedMemoryStatus& serverCmd =serverStatusOut;
    serverCmd.m_sendCastContactPointArgs.m_numContactPointsCopied = 0;

    //make a snapshot of the contact manifolds into individual contact points
    if (clientCmd.m_requestCastContactPointArguments.m_startingContactPointIndex == 0)
    {
        m_data->m_cachedCastContactPoints.resize(0);

        int mode = CAST_CONTACT_QUERY_MODE_REPORT_EXISTING_CONTACT_POINTS;

        if (clientCmd.m_updateFlags & CMD_REQUEST_CAST_CONTACT_POINT_HAS_QUERY_MODE)
        {
            mode = clientCmd.m_requestCastContactPointArguments.m_mode;
        }

        switch (mode)
        {
        case CAST_CONTACT_QUERY_MODE_REPORT_EXISTING_CONTACT_POINTS:
        {
            int numContactManifolds = m_data->m_dynamicsWorld->getDispatcher()->getNumManifolds();
            m_data->m_cachedCastContactPoints.reserve(numContactManifolds * 4);
            for (int i = 0; i < numContactManifolds; i++)
            {
                const btPersistentManifold* manifold = m_data->m_dynamicsWorld->getDispatcher()->getInternalManifoldPointer()[i];
                int linkIndexA = -1;
                ...
                //apply the filter, if the user provides it
                bool swap = false;
                if (clientCmd.m_requestCastContactPointArguments.m_objectAIndexFilter >= 0)
                {
                    if (clientCmd.m_requestCastContactPointArguments.m_objectAIndexFilter == objectIndexA)
                    {
                        swap = false;
                    }
                    else if (clientCmd.m_requestCastContactPointArguments.m_objectAIndexFilter == objectIndexB)
                    {
                        swap = true;
                    }
                    else
                ...
                //apply the second object filter, if the user provides it
                if (clientCmd.m_requestCastContactPointArguments.m_objectBIndexFilter >= 0)
                {
                    if (clientCmd.m_requestCastContactPointArguments.m_objectBIndexFilter != objectIndexB)
                    {
                        continue;
                    }
                }
                ...
                if (
                    (clientCmd.m_updateFlags & CMD_REQUEST_CAST_CONTACT_POINT_HAS_LINK_INDEX_A_FILTER) &&
                    clientCmd.m_requestCastContactPointArguments.m_linkIndexAIndexFilter != linkIndexA)
                {
                    continue;
                }

                if (
                    (clientCmd.m_updateFlags & CMD_REQUEST_CAST_CONTACT_POINT_HAS_LINK_INDEX_B_FILTER) &&
                    clientCmd.m_requestCastContactPointArguments.m_linkIndexBIndexFilter != linkIndexB)
                {
                    continue;
                }
                ...
                for (int p = 0; p < manifold->getNumContacts(); p++)
                {
                    ...
                    pt.m_normalForce = srcPt.getAppliedImpulse() / m_data->m_physicsDeltaTime;
                    //                                    pt.m_linearFrictionForce = srcPt.m_appliedImpulseLateral1;
                    m_data->m_cachedCastContactPoints.push_back(pt);
                }
            }
            break;
        }

        case CAST_CONTACT_QUERY_MODE_COMPUTE_CLOSEST_POINTS:
        {
            //todo(erwincoumans) compute closest points between all, and vs all, pair
            btScalar closestDistanceThreshold = 0.f;

            if (clientCmd.m_updateFlags & CMD_REQUEST_CAST_CONTACT_POINT_HAS_CLOSEST_DISTANCE_THRESHOLD)
            {
                closestDistanceThreshold = clientCmd.m_requestCastContactPointArguments.m_closestDistanceThreshold;
            }

            int bodyUniqueIdA = clientCmd.m_requestCastContactPointArguments.m_objectAIndexFilter;
            int bodyUniqueIdB = clientCmd.m_requestCastContactPointArguments.m_objectBIndexFilter;

            bool hasLinkIndexAFilter = (0!=(clientCmd.m_updateFlags & CMD_REQUEST_CAST_CONTACT_POINT_HAS_LINK_INDEX_A_FILTER));
            bool hasLinkIndexBFilter = (0!=(clientCmd.m_updateFlags & CMD_REQUEST_CAST_CONTACT_POINT_HAS_LINK_INDEX_B_FILTER));

            int linkIndexA  = clientCmd.m_requestCastContactPointArguments.m_linkIndexAIndexFilter;
            int linkIndexB = clientCmd.m_requestCastContactPointArguments.m_linkIndexBIndexFilter;
...

            {
                ...
                struct MyContactResultCallback : public btCollisionWorld::ContactResultCallback
                {
                    ...
                    MyContactResultCallback(btAlignedObjectArray<b3ContactPointData>& pointCache)
                    :m_cachedCastContactPoints(pointCache)
                    {
                    }
                    ...
                    virtual	btScalar	addSingleResult(btManifoldPoint& cp, const btCollisionObjectWrapper* colObj0Wrap, int partId0, int index0, const btCollisionObjectWrapper* colObj1Wrap, int partId1, int index1)
                    {
                        if (cp.m_distance1<=m_closestDistanceThreshold)
                        {
                            ...
                            pt.m_normalForce = srcPt.getAppliedImpulse() / m_deltaTime;
                            //                                    pt.m_linearFrictionForce = srcPt.m_appliedImpulseLateral1;
                            m_cachedCastContactPoints.push_back(pt);
                        }
                        return 1;
                ...
                MyContactResultCallback cb(m_data->m_cachedCastContactPoints);
    ...
    int numContactPoints = m_data->m_cachedCastContactPoints.size();
    ...
    int startContactPointIndex = clientCmd.m_requestCastContactPointArguments.m_startingContactPointIndex;
    int numContactPointBatch = btMin(numContactPoints,contactPointStorage);
    ...
    for (int i=startContactPointIndex;i<endContactPointIndex ;i++)
    {
        const b3ContactPointData& srcPt = m_data->m_cachedCastContactPoints[i];
        b3ContactPointData& destPt = contactData[serverCmd.m_sendCastContactPointArgs.m_numContactPointsCopied];
        destPt = srcPt;
        serverCmd.m_sendCastContactPointArgs.m_numContactPointsCopied++;
    }

    serverCmd.m_sendCastContactPointArgs.m_startingContactPointIndex = clientCmd.m_requestContactPointArguments.m_startingContactPointIndex;
    serverCmd.m_sendCastContactPointArgs.m_numRemainingContactPoints = numContactPoints - clientCmd.m_requestContactPointArguments.m_startingContactPointIndex - serverCmd.m_sendCastContactPointArgs.m_numContactPointsCopied;
    serverCmd.m_numDataStreamBytes = totalBytesPerContact * serverCmd.m_sendCastContactPointArgs.m_numContactPointsCopied;
    serverCmd.m_type = CMD_CONTACT_POINT_INFORMATION_COMPLETED; //CMD_CONTACT_POINT_INFORMATION_FAILED,
...
bool PhysicsServerCommandProcessor::processCommand(const struct SharedMemoryCommand& clientCmd, struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient, int bufferSizeInBytes )
{
    ...
    case CMD_REQUEST_CAST_CONTACT_POINT_INFORMATION:
        {
            hasStatus = processRequestCastContactpointInformationCommand(clientCmd,serverStatusOut,bufferServerToClient, bufferSizeInBytes);
            break;
        }
